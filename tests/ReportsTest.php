...
class ReportsTest extends TestCase
{
	...
	{
		$response
			= $this->getPlaidClient()->reports->createAssetReport([
		...
		$this->assertEquals(["access_token1", "access_token2"],
			$response->params->access_tokens);
	...
	{
		$response
			= $this->getPlaidClient()->reports->refreshAssetReport('asset_report_token',
			30);
		...
		$this->assertEquals("client_id", $response->params->client_id);
		$this->assertEquals("secret", $response->params->secret);
		$this->assertEquals("asset_report_token",
			$response->params->asset_report_token);
		$this->assertEquals(30, $response->params->days_requested);
		$this->assertEquals((object) [], $response->params->options);
	}
	...
	{
		$response
			= $this->getPlaidClient()->reports->filterAssetReport('asset_report_token',
			['account1', 'account2']);
		...
		$this->assertEquals(['account1', 'account2'],
			$response->params->account_ids_to_exclude);
	}

	public function test_get_asset_report(): void
	{
		$response
			= $this->getPlaidClient()->reports->getAssetReport('asset_report_token',
			true);
	...
	{
		$response
			= $this->getPlaidClient()->reports->getAssetReportPdf('asset_report_token');
	...
	{
		$response
			= $this->getPlaidClient()->reports->removeAssetReport('asset_report_token');
	...
	{
		$response
			= $this->getPlaidClient()->reports->createAssetReportAuditCopy('asset_report_token',
			'auditor_id');
	...
	{
		$response
			= $this->getPlaidClient()->reports->removeAssetReportAuditCopy('audit_copy_token');
		...
		$this->assertEquals("client_id", $response->params->client_id);
		$this->assertEquals("secret", $response->params->secret);
		$this->assertEquals("audit_copy_token",
			$response->params->audit_copy_token);
	}
}